...
#Linking the decoder input and output
decoder = tensorflow.keras.models.Model(decoder_input, decoder_output, name="decoder_model")
decoder.summary()

#Creating an input layer representing the input to the VAE encoder
vae_input = tensorflow.keras.layers.Input(shape=(img_size, img_size, num_channels), name="VAE_input")

#Built the output
vae_encoder_output = encoder(vae_input)

#Passing the output as input to the decoder
...
vae.fit(x_train, x_train, epochs=35, batch_size=32, shuffle=True, validation_data=(x_test, x_test))
